...
fornito la definizione $u(x(q, \tau), \tau) = \partial_a x(q, \tau) = \partial_{\tau}x(q, \tau) = \dot{x}(q, \tau)$.
...
\end{gather}
Definiamo la funzione $F(x(q, \tau))$ come una forza "efficace" di multistreaming. Nel dettaglio, $F(x(q, \tau)) = (\partial_q x) \int \delta_D[x(q, \tau)-x(q', \tau)]dq' -1$.
...
dipende dal tempo, allora è equivalente far agire l'operatore su $\xi$ anzichè su x. 
...
L'espansione proposta in (\ref{eqn:phiexp}) si ritrova in \cite{rampf} e permette di disporre del termine di quarto grado 
...
\end{equation}
dove le parentesi angolari simboleggiano una media pesata sul dominio delle $q$.
...
In Fig.(\ref{fig:fig1}) è rappresentato il moto della particelle nell'istante $\tau=1.002$, ovvero
...
\begin{center}
    \begin{figure}[H]
        ...
        nel pannello centrale e quello inferiore sono mappate rispettivamente $M(q, \tau)$ e $F(q, \tau)$.
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
dove la definizione di $D(q, \tau, c)$ è stata fornita in precedenza, mentre 
...
La (\ref{eqn:finale}) fornisce quindi l'andamento complessivo del dislocamento $\xi$. In figura Fig.(\ref{fig:fig2}) è proposto un confronto
...
Nel precedente capitolo si è ottenuta l'equazione del dislocamento $\xi_{PZA}(q,\tau)$, definita in tre
...
in espansione di Taylor la funzione $D$ per piccole deviazioni $\delta\tau>0$ intorno a $\tau_{com}$, a $q$ fissato. 
Da tale espansione si troverà che il termine dominante per $\xi_{PZA}$ è del tipo $\delta\tau^{5/2}$. 
Un ragionamento del tutto analogo si presenta se l'espansione invece si compie su $q$ fissando 
l'istante $\tau$ ed espandendo dunque intorno a $q_{comax}$ o $q_{comin}$:
...
Anche la derivata terza spaziale risulta essere discontinua, e l'andamento efficace di $\xi_{PZA}$ a $\tau$ fissato è di nuovo $q^{5/2}$.
Sia la prima singolarità di tipo (a) che quella di tipo (b) sono rappresentate nel pannello di sinistra di
Fig.(\ref{fig:fig2}), che riporta una soluzione con $c$=0.
...
non olomorfo (analitico). Questo tipo di singolarità si può osservare nel terzo pannello di Fig.(\ref{fig:fig2}), dove è
raffigurato l'andamento di $\xi_{PZA}$ con $c$=0.1.

Sempre in figura Fig.(\ref{fig:fig2}) è effettuata una comparazione tra i risultati analitici (linea continua) e una simulazione 
a N corpi della dinamica (punti): il codice usato per la simulazione tratta il caso unidimensionale, è simplettico
e si basa su un algoritmo che determina la forza di multistreaming, partendo da condizioni iniziali stabilite con
