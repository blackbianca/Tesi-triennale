...
iniziale lagrangiana $\bm{q}$ alla posizione corrente euleriana $\bm{x}$ al tempo cosmologico $\tau$ $\propto$ $t^{2/3}$.
...
da  $\delta \propto t^{2/3}$, $\bm{v} \propto t^{1/3}$ e $\phi = const$. Con questi andamenti, è
...
\end{equation}
Le traiettorie delle particelle, rettilinee e a velocità costante $\bm{u}$, sono descritte da
...
densità. Questo tipo di evento è detto \textit{caustica} e rappresenta il 
fenomeno che si registra quando due particelle con diverse coordinate lagrangiane iniziali 
$\bm{q}_1$ e $\bm{q}_2$ confluiscono nella stessa coordinata di campo $\bm{x}$ (\textit{shell-crossing}), dando luogo a densità
infinita. 
...
Una via percorribile è rappresentata dal problema variazionale come fu formulato da Peebels  (\cite{peebles}).
...
Questo metodo regge solamente in regime euleriano lineare $|(\rho-\rho_b)/\rho_b|\leq 1$, pertanto non si
...
Se $\Phi$ esiste ed è convesso sarà definita convessa anche la mappa inversa $\Theta(\bm{x}, t)$ tale che
...
in cui si cerca quale relazione tra $\bm{q}$ e $\bm{x}$ minimizza la funzione quadratica di costo 
